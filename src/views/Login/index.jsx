            ...
            <WingBlank>
                <Form>
                    <div className={styles.formItem}>
                        <Field name="username" className={styles.input} placeholder="请输入账号" type="text"/>
                    </div>
                    <ErrorMessage className={styles.error} component="div" name="username" />
                    <div className={styles.formItem}>
                        <Field name="password" className={styles.input} placeholder="请输入密码" type="password"/>
                    </div>
                    <ErrorMessage className={styles.error} component="span" name="password" />
...
export default withFormik({
    ...
    validationSchema: Yup.object().shape({
        username: Yup.string().matches(USERNAMEREG,'用户名必须是5-8位').required('账号为必填项'),
        password: Yup.string().matches(PASSWORDREG,'密码必须是5-12位').required('密码为必填项')
    }),
