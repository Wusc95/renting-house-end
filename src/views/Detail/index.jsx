...
export default class Detail extends Component {
  ...
  // 查看是否收藏
  async getIsFavorite() {
    const result = await this.axios.get(`/user/favorites/${this.props.match.params.id}`)
  ...
  favoriteOrNot = async () => {
    ...
    let result = null
    if (isFavorite) { // 代表现在是收藏状态，点击之后要取消收藏
      result = await this.axios.delete(`/user/favorites/${this.props.match.params.id}`)
    } else { // 之前没有收藏过，现在要进行收藏操作
      result = await this.axios.post(`/user/favorites/${this.props.match.params.id}`)
    }
